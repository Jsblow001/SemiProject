...
<div class="storeWrap">
  <h2 id="title">매장 오시는 길</h2>

  <div id="map"></div>

  <div id="latlngResult" class="latlngBox">
    지도에서 클릭하면 위도/경도가 표시됩니다.
  </div>
</div>
...
$(function(){
    ...
    var mapContainer = document.getElementById("map");
 	...
 	// 지도를 생성할때 필요한 기본 옵션
    var options = {
		center: new kakao.maps.LatLng(37.4932437695434, 127.079114775367), // 지도의 중심좌표. 반드시 존재해야함.
		<%--
    ...
    if(navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(function(position) {
       	...
       	//    console.log("현위치의 위도: "+latitude+", 현위치의 경도: "+longitude);
            ...
            var imageSrc = 'http://localhost:9090/SemiProject/img/pointerGreen.png'; 
         	// 마커이미지의 크기 
    ...
    $.ajax({
        url:"${pageContext.request.contextPath}/storeLocationJSON.sp",
        async:false, // !!!!! 지도는 비동기 통신이 아닌 동기 통신으로 해야 한다.!!!!!!
        dataType:"json",
        success:function(json){
        	console.log(JSON.stringify(json)); // JSON.stringify(json) 은 자바스크립트의 객체(배열)인 json 을 string 타입으로 변경시켜주는 것이다.
	        <%--   
        	...
        	$.each(json, function(index, item){
                ...
                "  </div>"+
                "  <div class='desc'>"+
                "    <img src='${pageContext.request.contextPath}/img/"+item.storeimg+"' />"+
    ...
    for (var i = 0; i < positionArr.length; i++) {
    	...
    	var marker = new kakao.maps.Marker({ 
     		 map: mapobj, 
        	 position: positionArr[i].latlng // positionArr[i].latlng 좌표에 마커를 생성 
       	//	 image: markerImage     // 마커이미지 설정
       	}); 
    	
    	// ==== 지도에 마커를 표시한다 ==== //
    	marker.setMap(mapobj); // 지도에 마커 표시
    	...
    	// ==== 마커 위에 인포윈도우 표시하기(이거 쓰면 안됨) ==== //
    //	infowindow.open(mapobj, marker);
    	
    	// ==== 인포윈도우를 가지고 있는 객체를 배열에 넣기 ==== //
 	...
 	// ================== 지도에 클릭 이벤트를 등록하기 시작 ======================= //
      ...
      kakao.maps.event.addListener(mapobj, 'click', function(mouseEvent) {         
          ...
          var htmlMessage = "<strong>클릭한 위치</strong> &nbsp; | &nbsp; 위도: " + lat + " &nbsp; / &nbsp; 경도: " + lng;

          document.getElementById("latlngResult").innerHTML = htmlMessage;
