...
@WebServlet(
      ...
      urlPatterns = { "*.sp" }, 
      initParams = { 
            @WebInitParam(name = "propertyConfig", value = "C:/git/SemiProject/SemiProject/src/main/webapp/Command.properties", description = "*.up 에 대한 클래스의 매핑파일")
      })
@MultipartConfig(
...
public class FrontController extends HttpServlet {
   ...
   public void init(ServletConfig config) throws ServletException {
     /*
         ...
         맨 처음에 자동적으로 실행되어지는 메서드가 init(ServletConfig config) 이다.
         여기서 중요한 것은 init(ServletConfig config) 메서드는 WAS(톰캣)가 구동되어진 후
         딱 1번만 init(ServletConfig config) 메서드가 실행되어지고, 그 이후에는 실행이 되지 않는다. 
        ...
        */   
          ...
          */
            ...
            if(className != null) {
               ...
               cmdMap.put(key, obj);
               // cmdMap 속에 obj 를 넣을때 key를 Command.properties 파일에 있는 URL 주소로 했다.!!!
